...
namespace iplmcfd {
      ...
      : m_initial_path(boost::filesystem::current_path())
   {
      ...
      // | Charm++ parameters |
      // +--------------------+
      po::options_description charm_param("Charm++ Parameters");
      charm_param.add_options()
         ("chares_x", po::value< int >(&chares_x)->default_value( 1 ),
         "Number of chares in the X dimension."  )
         ("chares_y", po::value< int >(&chares_y)->default_value( 1 ),
         "Number of chares in the Y dimension."  )
         ("chares_z", po::value< int >(&chares_z)->default_value( 1 ),
         "Number of chares in the Z dimension."  )
		 ("balance_init", po::value< int >(&balance_init)->default_value( 10000 ),
         "Iteration to run the load balancer for the first time." )
		 ("balance_freq", po::value< int >(&balance_freq)->default_value( 10000 ),
         "Frequency to load balance the simulation." );
      ...
      geom_param.add_options()
		...
		name_value_map air = boost::assign::map_list_of ( "O2", .21 ) ( "N2", .79 );
		po::options_description ref_param("Reference Parameters");
		ref_param.add_options()
         ("ref_L",       po::value< real >(&ref_L)->default_value( real(1) ),
         "Reference length [cm]." )
         ("ref_U",       po::value< real >(&ref_U)->default_value( real(1) ),
         "Reference velocity [cm/s]." )
         ("ref_p",       po::value< real >(&ref_p)->default_value( real(1) ),
         "Reference pressure [atm]." )
         ("ref_T",       po::value< real >(&ref_T)->default_value( real(1) ),
         "Reference temperature [K]." )
         ("ref_species", po::value< name_value_map >(&ref_species)->default_value( air ),
         "Reference mixture [mole fractions]." );
      ...
      cmdline_options.add( charm_param );
      ...
      config_file_options.add( charm_param );
      ...
      debug_wait = false;
      restart = false;
	  nchares = int_v(chares_x,chares_y,chares_z);

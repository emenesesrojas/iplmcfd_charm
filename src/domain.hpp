...
namespace iplmcfd {
  ...
  public:
    ...
    static const chare_t CHARE_OOB = static_cast<chare_t>(-1);
   ...
   { 
    ...
    // Converts the index of a chare into a chare_t
    chare_t ind2chare( int_v ind ) const {
      if (  m_chareind.subs_out_of_bounds(ind) ) return CHARE_OOB;
      return m_chareind.ind(ind);	
    }
    ...
    //! \returns the corresponding i,j,k index
    //!
    index_v chare2ind( const chare_t& I ) const
    {
      index_v sub;
      chare2ind( I, sub );
      return sub;
    ...
    //! \param[out] sub corresponding i,j,k index
    //!
   void chare2ind( const chare_t& I, index_v& ind ) const
   {
      assert( !m_chareind.ind_out_of_bounds(I) );
      m_chareind.sub( I, ind );
   }
  ...
  private:
    ...
    mytools::numeric::indsub<NDIM> m_chareind;
  };

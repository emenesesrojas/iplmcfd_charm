...
namespace iplmcfd {
  ...
  Iplmc::Iplmc( const simparam& p, const Domain& d, const Subdomain& s )
    ...
    m_domain(d), 
    m_subdomain(s),
    m_chem( p.chem, p.chem_therm, p.chem_tran ),
    m_cpr( m_chem ),
    m_dt( 0 ),
    m_time( 0 )
  {
    ...
    BOOST_FOREACH( Subdomain::gidchare_mp::const_reference gidchare, 
      m_subdomain.neighs() )
    {
      Cell cell( m_chem );
      cell.ctr.reset();
      m_neigh_cells.insert( cell_mp::value_type( gidchare.first, cell ) );
    }
  ...
  // +------------------+
	...
	// Rest of the original update_dt function after communication
	real_t Iplmc:: update_dt_end(real_t dt, real_t ke){
  ...
  void Iplmc::initialize()
  {
    ...
    boost::uniform_01< rng_t, real_t > 
      rand( rng_t( rng_t::result_type( m_subdomain.my_chare() ) ) );
...
//printf("nhomes = %zu\n",m_subdomain.nhomes()); 
  ...
  void Iplmc::update_fields()
  {
    ...
    BOOST_FOREACH( cell_mp::const_reference gidcell, m_cells )
    {
      ...
      m_colors[i]  = m_subdomain.my_chare();
      m_u[i]       = vel[XDIM];
      m_v[i]       = vel[YDIM];
      m_w[i]       = vel[ZDIM];
	...
	// pup function
	void Iplmc::pup(PUP::er &p){
		gid_t key;		

		p | m_cells;
		// packing/unpcaking all cells
/*		BOOST_FOREACH(cell_mp::reference gidcell, m_cells)
    	{
      		Cell& cell = gidcell.second;
			if(p.isPacking()){

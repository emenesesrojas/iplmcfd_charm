...
namespace iplmcfd {
  ...
  void Iplmc::position_step( real_t dt, std::size_t iter )
  {
    ...
    static normal_gen_t gaussian(
      rngengine_t( rngengine_t::result_type( m_subdomain.my_chare() ) ), 
      pdf_t(0.,1.)
      );
      ...
      while ( p != pEnd )
      {
        ...
        else
        {
          ...
          if ( ( it = m_cells.find( new_gid ) ) != m_cells.end() )
            ...
            BOOST_ASSERT( "unknown MoveCase, something bad happened, come fix"  );

          if ( movecase == INHOME )
          {
			//printf("INHOME PARTICLE %zu to %zu %zu\n",gidcell.first, new_gid, it->first);
		    Cell& new_cell = it->second;
            // offset particle position
            p->X -= shift;
            // put the particle in its new cell
            //printf("cell.contained.size() = %zu\n",cell.contained().size());
            //printf("new_cell.incoming.size() = %zu\n",new_cell.incoming().size());
            transfer( p++, cell, new_cell );
            //log.shifted_particle();
          }
          else if ( movecase == INNEIGH )
          {
			//printf("NEIGH PARTICLE\n");
            Cell& new_cell = it->second;
            // offset particle position
            p->X -= shift;
            // put the particle in its new cell
  ...
  // Rest of position_step method
  void Iplmc::position_step_end(){
    //activity_log& log = activity_log::instance();
